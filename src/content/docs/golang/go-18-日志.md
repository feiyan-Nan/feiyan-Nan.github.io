...
// 4 微秒: 01:23:23.123123.

10

Llongfile

// 8 绝对路径和行号: /a/b/c/d.go:23
...
// 16 文件名和行号: d.go:23. overrides Llongfile

12

LUTC

// 32 使用UTC（GMT），而不是本地时区

Lmsgprefix

// 64 默认前缀放行首，这个标记把前缀prefix放到消息
...
= Ldate | Ltime // 3 initial values for the standard
...
mu
```
sync.Mutex // ensures atomic writes; protects the following
...
// prefix on each line to identify the logger (but
...
// atomic boolean: whether out == io.Discard
...
log1.Debug().Msg("log1的Debug级别消息")

// 不能输出

22

log1.Warn().Msg("log1的Warn级别消息")

// 输出

23

log1.Error().Msg("log1的Error级别消息")
...
```go
// {"level":"info","time":1223947070} 全局Logger
```
zerolog.TimeFieldFormat = zerolog.TimeFormatUnix
